...
$LogFolder = "$env:userprofile\desktop\logs"
 ...
 ForEach($User in $Users)
   ...
   $OU="CN=users, DC=lan-autogyre,DC=local" # Choose an Ou where users will be created # Running cmd will show all OU's Get-ADOrganizationalUnit -Filter * | Select-Object -Property DistinguishedName| Out-GridView -PassThru| Select-Object -ExpandProperty DistinguishedName
